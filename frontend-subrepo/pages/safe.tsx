...
const Safe: React.FC<Props> = ({ connectWallet, connected, address, errmsg, role }) => {
    ...
    const readSafes = async (address: string) => {
        try {
            const res = await axios.get(apiHost + "safe/" + address);
            if (res.status == 200) {
                setSafes(res.data)
            } else {
                                ...
                                return (
                                    <Link key={index} href={"/safe/" + item['_id']} >
                                        <SafeItem
                                            ...
                                            amount={123} approval={item['approver'] + '/3'}
