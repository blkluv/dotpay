...
const Index: React.FC<Props> = ({ connectWallet, connected, address, errmsg, role }) => {
    ...
    const readRequest = async () => {
        try {
            const res = await axios.get(apiHost + 'request/' + address)
                ...
                <div className='list-sect'>
                    {
                        requests.map((item, index) => {
                            return (
                                <ReqItem key={index} amount={item['amount']['amount'] + " " + item['amount']['token']} address={beatifyAddress(item['owner'])} hashDate={'Created on ' + item['created']} byDate={item['program']['program']} by={item['name']} id={item['_id']} />
                            )
                        })
                    }
                </div>
